...
frappe.provide("shoperprime.tally_migration");
...
frappe.ui.form.on("Tally Migration", {
	...
	refresh: function (frm) {
		...
		shoperprime.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		shoperprime.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	shoperprime_company: function (frm) {
		frappe.db.exists("Company", frm.doc.shoperprime_company).then(exists => {
			if (exists) {
				frappe.msgprint(
					__("Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts", [frm.doc.shoperprime_company]),
				);
			}
		});
	},
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = shoperprime.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = shoperprime.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = shoperprime.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = shoperprime.tally_migration.fixed_errors_log;
...
shoperprime.tally_migration.getError = (traceback) => {
...
shoperprime.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function(key, value){
		if (value === "" || value === null){
			...
			shoperprime.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k,v) { shoperprime.tally_migration.cleanDoc(v); });
		}
	});
	return temp;
}

shoperprime.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = shoperprime.tally_migration.failed_import_log;
	let fixed_log = shoperprime.tally_migration.fixed_errors_log;

	let modified_fixed_log = fixed_log.filter(row => {
		if (!frappe.utils.deep_equal(shoperprime.tally_migration.cleanDoc(row.doc), document)) {
			return row
		}
	});
...
shoperprime.tally_migration.resolve = (document) => {
...
shoperprime.tally_migration.create_new_doc = (document) => {
	/* Mark as resolved and create new document */
	shoperprime.tally_migration.resolve(document);
	return frappe.call({
		type: "POST",
		method: 'shoperprime.shoperprime_integrations.doctype.tally_migration.tally_migration.new_doc',
		args: {
			document
		},
		freeze: true,
...
shoperprime.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let error_message = shoperprime.tally_migration.getError(traceback);
			index++;
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(shoperprime.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='shoperprime.tally_migration.create_new_doc(${JSON.stringify(doc)})'>
					${__("Create Document")}
				</button>`

			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='shoperprime.tally_migration.unresolve(${JSON.stringify(doc)})'>
