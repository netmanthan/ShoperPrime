		...
		frm.add_custom_button(__('Generate Secret'), () => {
			frappe.confirm(
				...
				() => {
					frappe.call({
						type:"POST",
						method:"shoperprime.shoperprime_integrations.doctype.woocommerce_settings.woocommerce_settings.generate_secret",
					}).done(() => {
						frm.reload_doc();
					}).fail(() => {
...
frappe.ui.form.on("Woocommerce Settings", "onload", function () {
	frappe.call({
		method: "shoperprime.shoperprime_integrations.doctype.woocommerce_settings.woocommerce_settings.get_series",
		callback: function (r) {
