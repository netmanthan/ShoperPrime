...
from shoperprime.stock.utils import get_stock_value_on
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = (
			"paid_from" if shoperprime.get_party_account_type(args.party_type) == "Receivable" else "paid_to"
		)
...
def get_outstanding_invoices(
	...
	if account:
		...
		party_account_type = shoperprime.get_party_account_type(party_type)
...
def get_children(doctype, parent, company, is_root=False):
	from shoperprime.accounts.report.financial_statements import sort_accounts
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	from shoperprime.setup.setup_wizard.operations.install_fixtures import create_bank_account
...
def get_coa(doctype, parent, is_root, chart=None):
	from shoperprime.accounts.doctype.account.chart_of_accounts.chart_of_accounts import (
...
def repost_gle_for_stock_vouchers(
	...
	from shoperprime.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	journal_account = frappe._dict(
		{
			...
			"exchange_rate": 0,
			"cost_center": cost_center or shoperprime.get_default_cost_center(company),
