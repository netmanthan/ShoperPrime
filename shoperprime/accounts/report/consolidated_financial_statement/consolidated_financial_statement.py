...
import shoperprime
from shoperprime.accounts.report.balance_sheet.balance_sheet import (
	get_chart_data,
...
from shoperprime.accounts.report.cash_flow.cash_flow import (
	add_total_row_account,
	get_account_type_based_gl_data,
	get_cash_flow_accounts,
)
from shoperprime.accounts.report.cash_flow.cash_flow import get_report_summary as get_cash_flow_summary
from shoperprime.accounts.report.financial_statements import (
	filter_out_zero_value_rows,
	get_fiscal_year_data,
	sort_accounts,
)
from shoperprime.accounts.report.profit_and_loss_statement.profit_and_loss_statement import (
...
from shoperprime.accounts.report.utils import convert, convert_to_presentation_currency
...
def execute(filters=None):
	...
	if filters.get("report") == "Balance Sheet":
		...
		if cint(frappe.db.get_single_value("Accounts Settings", "use_custom_cash_flow")):
			from shoperprime.accounts.report.cash_flow.custom_cash_flow import execute as execute_custom
...
def get_columns(companies, filters):
	...
	for company in companies:
		...
		if not currency:
			currency = shoperprime.get_company_currency(company)
			...
			if d:
				...
				for company in companies:
					...
					if (
						...
						parent_company_currency = shoperprime.get_company_currency(d.company)
						child_company_currency = shoperprime.get_company_currency(entry.company)
