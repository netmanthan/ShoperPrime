...
from shoperprime.accounts.general_ledger import (
	get_round_off_account_and_cost_center,
	make_gl_entries,
	make_reverse_gl_entries,
	merge_similar_entries,
)
from shoperprime.accounts.party import get_due_date, get_party_account
from shoperprime.accounts.utils import get_account_currency, get_fiscal_year
from shoperprime.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
from shoperprime.assets.doctype.asset_category.asset_category import get_asset_category_account
from shoperprime.buying.utils import check_on_hold_or_closed_status
from shoperprime.controllers.accounts_controller import validate_account_head
from shoperprime.controllers.buying_controller import BuyingController
...
class PurchaseInvoice(BuyingController):
	...
	def on_submit(self):
		...
		if self.update_stock == 1:
			...
			from shoperprime.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit
	...
	def get_gl_entries(self, warehouse_account=None):
		self.auto_accounting_for_stock = shoperprime.is_perpetual_inventory_enabled(self.company)
		...
		for item in self.get("items"):
			if item.is_fixed_asset:
				...
				if not self.update_stock:
					...
					if item.item_tax_amount and not cint(shoperprime.is_perpetual_inventory_enabled(self.company)):
						if not eiiav_account or not asset_eiiav_currency:
							eiiav_account = self.get_company_default("expenses_included_in_asset_valuation")
							asset_eiiav_currency = get_account_currency(eiiav_account)
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		for pr in set(updated_pr):
			from shoperprime.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage
