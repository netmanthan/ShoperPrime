...
import shoperprime
from shoperprime.accounts.deferred_revenue import get_deferred_booking_accounts
...
from shoperprime.controllers.accounts_controller import AccountsController
...
class JournalEntry(AccountsController):
	...
	def apply_tax_withholding(self):
		from shoperprime.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from shoperprime.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": shoperprime.get_default_cost_center(self.company),
						},
					)
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	from shoperprime.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
...
def get_outstanding(args):
	...
	company_currency = shoperprime.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_balance_and_party_type(
	...
	company_currency = shoperprime.get_company_currency(company)
	account_details = frappe.db.get_value(
