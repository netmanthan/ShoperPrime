...
$.extend(shopping_cart, {
	show_shoppingcart_dropdown: function() {
		$(".shopping-cart").on('shown.bs.dropdown', function() {
			if (!$('.shopping-cart-menu .cart-container').length) {
				return frappe.call({
					method: 'shoperprime.e_commerce.shopping_cart.cart.get_shopping_cart_menu',
					callback: function(r) {
						if (r.message) {
	...
	update_cart: function(opts) {
		if (frappe.session.user==="Guest") {
			...
			return frappe.call({
				type: "POST",
				method: "shoperprime.e_commerce.shopping_cart.cart.update_cart",
				args: {
	...
	show_cart_navbar: function () {
		frappe.call({
			method: "shoperprime.e_commerce.doctype.e_commerce_settings.e_commerce_settings.is_cart_enabled",
			callback: function(r) {
	...
	bind_add_to_cart_action() {
		$('.page_content').on('click', '.btn-add-to-cart-list', (e) => {
			...
			shoperprime.e_commerce.shopping_cart.update_cart({
				item_code,
				qty: 1
			});
