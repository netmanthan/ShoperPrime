...
import shoperprime
from shoperprime.accounts.general_ledger import process_gl_map
from shoperprime.controllers.taxes_and_totals import init_landed_taxes_and_totals
from shoperprime.manufacturing.doctype.bom.bom import add_additional_cost, validate_bom_no
from shoperprime.setup.doctype.brand.brand import get_brand_defaults
from shoperprime.setup.doctype.item_group.item_group import get_item_group_defaults
from shoperprime.stock.doctype.batch.batch import get_batch_no, get_batch_qty, set_batch_nos
from shoperprime.stock.doctype.item.item import get_item_defaults
...
from shoperprime.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from shoperprime.stock.utils import get_bin, get_incoming_rate
...
from shoperprime.controllers.stock_controller import StockController
...
class StockEntry(StockController):
	...
	def before_validate(self):
		from shoperprime.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate_difference_account(self):
		if not cint(shoperprime.is_perpetual_inventory_enabled(self.company)):
			return
	...
	def set_actual_qty(self):
		from shoperprime.stock.stock_ledger import is_negative_stock_allowed
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				d.basic_rate = get_valuation_rate(
					...
					currency=shoperprime.get_company_currency(self.company),
					company=self.company,
	...
	def get_bom_raw_materials(self, qty):
		from shoperprime.manufacturing.doctype.bom.bom import get_bom_items_as_dict
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from shoperprime.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from shoperprime.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def get_items_from_subcontract_order(source_name, target_doc=None):
	from shoperprime.controllers.subcontracting_controller import make_rm_stock_entry
