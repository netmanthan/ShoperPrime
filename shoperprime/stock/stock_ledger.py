...
import shoperprime
from shoperprime.stock.doctype.bin.bin import update_qty as update_bin_qty
from shoperprime.stock.utils import (
	get_incoming_outgoing_rate_for_cancel,
	get_or_make_bin,
	get_valuation_method,
)
from shoperprime.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from shoperprime.controllers.stock_controller import future_sle_exists
...
class update_entries_after(object):
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		# Sales and Purchase Return
			...
			if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from shoperprime.controllers.sales_and_purchase_return import (
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=shoperprime.get_company_currency(sle.company),
			company=sle.company,
			batch_no=sle.batch_no,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(shoperprime.is_perpetual_inventory_enabled(company))
	):
