...
{% include 'shoperprime/loan_management/loan_common.js' %};
...
frappe.ui.form.on('Loan', {
	...
	make_loan_disbursement: function (frm) {
		frappe.call({
			...
			method: "shoperprime.loan_management.doctype.loan.loan.make_loan_disbursement",
			callback: function (r) {
				if (r.message)
	...
	make_repayment_entry: function(frm) {
		frappe.call({
			...
			method: "shoperprime.loan_management.doctype.loan.loan.make_repayment_entry",
			callback: function (r) {
				if (r.message)
					var doc = frappe.model.sync(r.message)[0];
				frappe.set_route("Form", doc.doctype, doc.name);
	...
	make_loan_write_off_entry: function(frm) {
		frappe.call({
			...
			method: "shoperprime.loan_management.doctype.loan.loan.make_loan_write_off",
			callback: function (r) {
				if (r.message)
					var doc = frappe.model.sync(r.message)[0];
				frappe.set_route("Form", doc.doctype, doc.name);
	...
	make_loan_refund: function(frm) {
		frappe.call({
			...
			method: "shoperprime.loan_management.doctype.loan.loan.make_refund_jv",
			callback: function (r) {
				if (r.message) {
	...
	close_unsecured_term_loan: function(frm) {
		frappe.call({
			...
			method: "shoperprime.loan_management.doctype.loan.loan.close_unsecured_term_loan",
			callback: function () {
				frm.refresh();
			}
		})
		...
		frappe.confirm(__("Do you really want to close this loan"),
			function() {
				frappe.call({
					...
					method: "shoperprime.loan_management.doctype.loan.loan.request_loan_closure",
					callback: function() {
						frm.reload_doc();
					}
				});
	...
	create_loan_security_unpledge: function(frm) {
		frappe.call({
			method: "shoperprime.loan_management.doctype.loan.loan.unpledge_security",
			args : {
				"loan": frm.doc.name,
				"as_dict": 1
			},
	...
	loan_application: function (frm) {
		if(frm.doc.loan_application){
			return frappe.call({
				method: "shoperprime.loan_management.doctype.loan.loan.get_loan_application",
				args: {
