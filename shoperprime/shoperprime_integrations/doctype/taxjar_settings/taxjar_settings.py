...
from shoperprime.shoperprime_integrations.taxjar_integration import get_client
	...
	def on_update(self):
		...
		if TAXJAR_CREATE_TRANSACTIONS or TAXJAR_CALCULATE_TAX or TAXJAR_SANDBOX_MODE:
			if not fields_already_exist:
				...
				make_custom_fields()
				add_permissions()
				frappe.enqueue("shoperprime.regional.united_states.setup.add_product_tax_categories", now=False)
