...
class QuickBooksMigrator(Document):
	...
	def on_update(self):
		if self.company:
			# We need a Cost Center corresponding to the selected shoperprime Company
	...
	def _migrate(self):
		try:
			...
			# QuickBooks data can have transactions that do not fall in existing fiscal years in shoperprime
	...
	def _save_account(self, account):
		...
		# Map Quickbooks Account Types to shoperprime root_accunts and and root_type
		try:
			if not frappe.db.exists(
